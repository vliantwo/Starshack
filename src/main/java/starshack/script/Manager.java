...
import starshack.script.model.Image;
...
public class Manager extends Module {
    ...
    public Manager() {
        ...
        this.registerSetting(new ButtonSetting("Load scripts", () -> {
            if (Stars.scriptManager.compiler == null) {
                ...
                if (Utils.timeBetween(this.lastLoad, currentTimeMillis) > 1500) {
                    ...
                    Stars.scriptManager.loadScripts();
                    if (Stars.scriptManager.scripts.isEmpty()) {
                        Utils.sendMessage("&7No scripts found.");
                    } else {
                        double timeTaken = Utils.round((System.currentTimeMillis() - currentTimeMillis) / 1000.0, 1);
                        Utils.sendMessage("&7Loaded &b" + Stars.scriptManager.scripts.size() + " &7script" + ((Stars.scriptManager.scripts.size() == 1) ? "" : "s") + " in &b" + Utils.asWholeNum(timeTaken) + "&7s.");
                    }
                    Entity.clearCache();
        ...
        this.registerSetting(new ButtonSetting("Open folder", () -> {
            try {
                Desktop.getDesktop().open(Stars.scriptManager.directory);
            } catch (IOException ex) {
                Stars.scriptManager.directory.mkdirs();
    ...
    private void createScript() {
        if (Stars.scriptManager == null) {
            return;
        }

        String scriptName = Stars.scriptManager.createScript(createScriptName.getText());
        if (scriptName != null) {
