...
public class Script {
    ...
    public boolean run() {
        try {
            ...
            final File file = new File(Stars.scriptManager.COMPILED_DIR);
            if (!file.exists() || !file.isDirectory()) {
                file.mkdir();
            }
            if (Stars.scriptManager.compiler == null) {
                return false;
            }
            final ScriptDiagnosticListener bp = new ScriptDiagnosticListener();
            final StandardJavaFileManager fileManager = Stars.scriptManager.compiler.getStandardFileManager(bp, null, null);
            final ArrayList<String> compilationOptions = new ArrayList<>();
            compilationOptions.add("-d");
            compilationOptions.add(Stars.scriptManager.COMPILED_DIR);
            compilationOptions.add("-XDuseUnsharedTable");
            if (!(boolean) Launch.blackboard.get("fml.deobfuscatedEnvironment")) {
                compilationOptions.add("-classpath");
                String s = Stars.scriptManager.jarPath;
                try {
            ...
            boolean success = Stars.scriptManager.compiler.getTask(null, fileManager, bp, compilationOptions, null, Arrays.asList(new JavaSourceFromString(this.scriptName, this.codeStr, this.STARTING_LINE))).call();
            if (!success) {
                this.error = true;
                return false;
            }
    ...
    public void delete() {
        this.clazz = null;
        this.instance = null;
        final File file = new File(Stars.scriptManager.COMPILED_DIR + File.separator + this.scriptName + ".class");
        if (file.exists()) {
            file.delete();
        }
    }
    ...
    public void setCode(String code) {
        ...
        Iterator<String> iterator = Stars.scriptManager.imports.iterator();
        while (iterator.hasNext()) {
            STARTING_LINE++;
            fileCodeContents.append("import ").append(iterator.next()).append(";\n");
        }
        fileCodeContents.append("import starshack.script.model.*;\n");
        fileCodeContents.append("import starshack.script.packet.clientbound.*;\n");
        fileCodeContents.append("import starshack.script.packet.serverbound.*;\n");
