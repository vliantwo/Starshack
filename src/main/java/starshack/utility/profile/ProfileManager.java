...
import starshack.module.impl.combat.Velocity;
import starshack.module.impl.player.HideWindow;
...
public class ProfileManager implements IMinecraftInstance {
    ...
    public void saveProfile(Profile profile) {
        ...
        for (Module module : Stars.moduleManager.getModules()) {
        ...
        if (Stars.scriptManager != null && Stars.scriptManager.scripts != null) {
    ...
    public void loadProfile(String name) {
        ...
        for (File file : getProfileFiles()) {
            ...
            try (FileReader fileReader = new FileReader(file)) {
                ...
                JsonObject legacyJumpResetData = null;
                JsonObject legacyAutoBlockData = null;
                ...
                for (JsonElement moduleJson : modules) {
                    ...
                    if ("Jump Reset".equalsIgnoreCase(moduleName) || "JumpReset".equalsIgnoreCase(moduleName)) {
                        legacyJumpResetData = moduleInformation;
                        continue;
                    }

                    if ("Auto Block".equalsIgnoreCase(moduleName) || "Autoblock".equalsIgnoreCase(moduleName)) {
                        legacyAutoBlockData = moduleInformation;
                        continue;
                    }
                ...
                migrateLegacyAutoBlock(legacyAutoBlockData, loadedModuleData);
                migrateLegacyJumpReset(legacyJumpResetData, loadedModuleData, requestedModuleStates);
                if (!loadedRelationshipsState && ModuleManager.relationships != null && Stars.playerRelationsManager != null) {
                    RequestedModuleState relationshipsState = requestedModuleStates.get(ModuleManager.relationships);
                    if (relationshipsState != null) {
                        relationshipsState.enabled = Stars.playerRelationsManager.isActive();
                    }
                }
                ...
                for (Map.Entry<Module, JsonObject> entry : loadedModuleData.entrySet()) {
                    ...
                    if (module.getName().equals("HUD")) {
                        ...
                        for (Map.Entry<String, JsonElement> setting : moduleInformation.entrySet()) {
                            String settingName = migrateCategoryName(setting.getKey());
                ...
                Stars.currentProfile = getProfile(profileName);

                boolean loadGuiPositions = Settings.loadGuiPositions.isToggled();
                Stars.clickGui.refreshAfterProfileLoad();
                if (loadGuiPositions) {
                    for (CategoryComponent c : ClickGui.categories) {
                ...
                MinecraftForge.EVENT_BUS.post(new PostProfileLoadEvent(Stars.currentProfile.getName()));
                return;
            } catch (Exception e) {
                failedMessage("load", profileName);
                e.printStackTrace();
    ...
    private static void migrateLegacyAutoBlock(JsonObject legacyData, Map<Module, JsonObject> loadedModuleData) {
        if (legacyData == null || ModuleManager.killAura == null) {
            return;
        }

        JsonObject killAuraData = loadedModuleData.get(ModuleManager.killAura);
        if (killAuraData == null) {
            killAuraData = new JsonObject();
            killAuraData.addProperty("name", ModuleManager.killAura.getName());
            loadedModuleData.put(ModuleManager.killAura, killAuraData);
        }

        copyLegacySetting(legacyData, killAuraData, "enabled", "Auto Block.Enable");
        copyLegacySetting(legacyData, killAuraData, "Range", "Auto Block.Range");
        copyLegacySetting(legacyData, killAuraData, "Require right mouse", "Auto Block.Require press");
        copyLegacySetting(legacyData, killAuraData, "Ignore teammates", "Auto Block.Ignore teammates for blocking");
    }

    private static void copyLegacySetting(JsonObject source, JsonObject target, String oldKey, String newKey) {
        if (!target.has(newKey) && source.has(oldKey)) {
            target.add(newKey, source.get(oldKey));
        }
    }

    private List<Module> getLoadableModules() {
        List<Module> loadableModules = new ArrayList<Module>(Stars.getModuleManager().getModules());
        if (Stars.scriptManager != null && Stars.scriptManager.scripts != null) {
            loadableModules.addAll(Stars.scriptManager.scripts.values());
        }
    ...
    private void migrateLegacyJumpReset(JsonObject legacyData, Map<Module, JsonObject> loadedModuleData,
                                        Map<Module, RequestedModuleState> requestedModuleStates) {
        Velocity velocity = ModuleManager.velocity;
        if (legacyData == null || velocity == null) {
            return;
        }

        JsonObject velocityData = loadedModuleData.get(velocity);
        if (velocityData == null) {
            velocityData = new JsonObject();
            velocityData.addProperty("name", velocity.getName());
            loadedModuleData.put(velocity, velocityData);
        }

        copyPrimitive(legacyData, "Chance", velocityData, "Jump chance");
        copyPrimitive(legacyData, "Require mouse down", velocityData, "Require mouse down");
        copyPrimitive(legacyData, "Require moving forward", velocityData, "Require moving forward");
        copyPrimitive(legacyData, "Require aim", velocityData, "Require aim");

        if (!legacyData.has("enabled") || !legacyData.get("enabled").getAsBoolean()) {
            return;
        }

        velocityData.addProperty("Mode", 1);
        RequestedModuleState requestedState = requestedModuleStates.get(velocity);
        if (requestedState != null) {
            requestedState.enabled = true;
            if (legacyData.has("keybind")) {
                requestedState.keybind = legacyData.get("keybind").getAsInt();
            }
            if (legacyData.has("hidden")) {
                requestedState.hidden = legacyData.get("hidden").getAsBoolean();
            }
        }
    }

    private void copyPrimitive(JsonObject source, String sourceKey, JsonObject destination, String destinationKey) {
        if (source.has(sourceKey) && source.get(sourceKey).isJsonPrimitive()) {
            destination.add(destinationKey, source.get(sourceKey));
        }
    }
    ...
    public boolean deleteProfile(String name) {
        ...
        boolean wasCurrentProfile = removedProfile != null && Stars.currentProfile == removedProfile;
        if (removedProfile != null) {
            profiles.remove(removedProfile);
        }
        if (wasCurrentProfile) {
            Stars.currentProfile = null;
        }
    ...
    public void loadProfiles() {
        String currentProfileName = Stars.currentProfile != null ? Stars.currentProfile.getName() : null;
        boolean currentProfileSaved = Stars.currentProfile == null || Stars.currentProfile.getModule().saved;
        profiles.clear();
        ...
        if (currentProfileName != null) {
            Stars.currentProfile = getProfile(currentProfileName);
            if (Stars.currentProfile != null) {
                Stars.currentProfile.getModule().saved = currentProfileSaved;
            }
        }
        refreshProfileModules();
    ...
    void refreshProfileModules() {
        Module managerModule = ModuleManager.getModule(starshack.utility.profile.Manager.class);
    ...
    private static String migrateCategoryName(String categoryName) {
        if ("render".equals(categoryName)) return "visuals";
        if ("world".equals(categoryName)) return "exploits";
        if ("minigames".equals(categoryName) || "fun".equals(categoryName) || "other".equals(categoryName))
            return "misc";
        if ("client".equals(categoryName) || "profiles".equals(categoryName)) return "configs";
        return categoryName;
    }
