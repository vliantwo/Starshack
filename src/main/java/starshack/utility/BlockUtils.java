...
public class BlockUtils implements IMinecraftInstance {
    ...
    public static Vec3 getFaceCenter(BlockPos block, EnumFacing face) {
        ...
        switch (face) {
            case UP:
                return new Vec3(cx, block.getY() + 1 - eps, cz);
            case DOWN:
                return new Vec3(cx, block.getY() + eps, cz);
            case NORTH:
                return new Vec3(cx, cy, block.getZ() + eps);
            case SOUTH:
                return new Vec3(cx, cy, block.getZ() + 1 - eps);
            case EAST:
                return new Vec3(block.getX() + 1 - eps, cy, cz);
            case WEST:
                return new Vec3(block.getX() + eps, cy, cz);
            default:
