...
import starshack.utility.profile.ProfileModule;
...
public class ModuleComponent extends Component {
    ...
    private static final int HOVER_COLOR = new Color(79, 84, 92, ORIGINAL_HOVER_ALPHA).getRGB();
    private static final int UNSAVED_COLOR = new Color(88, 101, 242).getRGB();
    private static final int INVALID_COLOR = new Color(255, 80, 80).getRGB();
    private static final int ENABLED_COLOR = new Color(88, 101, 242).getRGB();
    private static final int DISABLED_COLOR = new Color(185, 187, 190).getRGB();
    ...
    public ModuleComponent(Module mod, CategoryComponent p, float yPos) {
        ...
        this.categoryManager = mod instanceof starshack.script.Manager;
    ...
    private void rebuildSettingsList() {
        ...
        if (mod != null && !mod.getSettings().isEmpty()) {
            for (Setting v : mod.getSettings()) {
                ...
                if (v instanceof SliderSetting) {
                    ...
                    y += slc.getHeightF();
                } else if (v instanceof starshack.module.setting.impl.BlockListSetting) {
                    starshack.module.setting.impl.BlockListSetting bls = (starshack.module.setting.impl.BlockListSetting) v;
        ...
        if (!categoryManager && !(mod instanceof Manager)) {
    ...
    public void render() {
        ...
        if (this.mod instanceof ProfileModule && !((ProfileModule) this.mod).saved && Stars.currentProfile != null && Stars.currentProfile.getModule() == this.mod) {
    ...
    public boolean onClick(int x, int y, int mouse) {
        if (hasModuleHeader() && this.overModuleName(x, y) && mouse == 0 && this.mod.canBeEnabled()) {
            this.mod.toggle();
            if (!(this.mod instanceof ProfileModule)) {
