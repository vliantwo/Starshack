...
import starshack.clickgui.animation.ScrollOffsetAnimation;
...
public class CategoryComponent {
    ...
    public float big;
    /**
     * True when the category is hosted by the single-window Eclipse ClickGUI.
     */
    private boolean embedded;
    private float embeddedContentHeight;
    ...
    public CategoryComponent(Module.category category) {
        ...
        for (Module mod : Stars.getModuleManager().inCategory(this.category)) {
    ...
    public void reloadModules(boolean isProfile) {
        ...
        if (this.category == Module.category.configs && isProfile) {
            for (Module module : Stars.getModuleManager().inCategory(this.category)) {
                ModuleComponent component = new ModuleComponent(module, this, moduleRenderY);
                component.restoreOpenState(Boolean.TRUE.equals(openStates.get(module.getName())));
                this.modules.add(component);
                moduleRenderY += 16;
            }
        } else if (this.category == Module.category.scripts && !isProfile) {
            ModuleComponent manager = new ModuleComponent(isProfile ? new Manager() : new starshack.script.Manager(), this, moduleRenderY);
            manager.restoreOpenState(Boolean.TRUE.equals(openStates.get(manager.mod.getName())));
            this.modules.add(manager);

            if ((Stars.profileManager == null && isProfile) || (Stars.scriptManager == null && !isProfile)) {
                return;
            }

            Collection<Module> modulesCollection = Stars.scriptManager.scripts.values();
    ...
    public void render(FontRenderer renderer) {
        ...
        if (embedded) {
            RenderUtils.drawRect(this.x, this.y, this.x + this.width, this.y + this.titleHeight + 3, 0xFF2F3136);
            titleRenderer.drawString(this.category.name(), this.x + 8, this.y + 4, CATEGORY_NAME_COLOR, false);
        } else {
    ...
    public void setEmbeddedLayout(float x, float y, float width, float contentHeight) {
        this.embedded = true;
        this.embeddedContentHeight = Math.max(32.0f, contentHeight);
        this.setX(x, false);
        this.setY(y, false);
        this.width = Math.max(120.0f, width);
        this.opened = true;
    }

    public void clearEmbeddedLayout() {
        this.embedded = false;
        this.embeddedContentHeight = 0.0f;
    }
    ...
    private CategoryLayoutMetrics computeLayoutMetrics(boolean updateModuleOffsets) {
        ...
        float maxModulesHeight = embedded
                ? embeddedContentHeight - this.titleHeight - 4
                : (this.screenHeight * 0.9f) - this.titleHeight - 4;
