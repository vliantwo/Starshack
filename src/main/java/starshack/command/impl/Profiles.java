...
public class Profiles extends Command {
    ...
    public void execute(CommandInput input) {
        if (input.argumentCount() == 0) {
            List<Profile> profiles = Stars.profileManager.profiles;
            if (profiles.isEmpty()) {
            ...
            for (Profile profile : profiles) {
                replyWithHeader(" &7" + profile.getName() + (profile == Stars.currentProfile ? " &7(&bcurrent&7)" : ""));
            }
            return;
        }
    ...
    private void handleSave(CommandInput input) {
        if (input.argumentCount() < 2) {
            if (Stars.currentProfile != null) {
                Utils.sendMessage("&7Saved profile: &b" + Stars.currentProfile);
        ...
        Profile savedProfile = Stars.profileManager.getProfile(saveName);
        if (savedProfile == null) {
            savedProfile = Stars.profileManager.createProfile(saveName, 0);
            if (savedProfile == null) {
                return;
            }
        } else {
            Stars.profileManager.saveProfile(savedProfile);
        }
    ...
    private void handleLoad(CommandInput input) {
        ...
        if (Stars.profileManager.getProfile(loadName) == null) {
            replyWithHeader("&b" + loadName + " &7does not exist");
            return;
        }

        Stars.profileManager.loadProfile(loadName);
    ...
    private void handleDelete(CommandInput input) {
        ...
        if (Stars.profileManager.getProfile(deleteName) == null) {
            replyWithHeader("&cProfile &b" + deleteName + " &7does not exist");
            return;
        }

        Stars.profileManager.deleteProfile(deleteName);
    ...
    private void handleRename(CommandInput input) {
        ...
        Profile oldProfile = Stars.profileManager.getProfile(oldName);
        if (oldProfile == null) {
            replyWithHeader("&b" + oldName + " &7does not exist");
            return;
        }

        if (Stars.profileManager.getProfile(newName) != null) {
            replyWithHeader("&b" + newName + " &7already exists");
            return;
        }

        if (!Stars.profileManager.renameProfile(oldProfile, newName)) {
            return;
        }
