...
public class LongJump extends Module {
    ...
    public void onPreUpdate(PreUpdateEvent e) {
        ...
        if (!function) {
            if (manual.isToggled() && !enabled) {
                boolean velocityAllowed = !onlyWithVelocity.isToggled()
                        || ModuleManager.velocity != null && ModuleManager.velocity.isEnabled();
                if (velocityAllowed && ModuleUtils.threwFireballLow) {
        ...
        if (enabled) {
            ...
            boolean floatMode = mode.getInput() == 0;
            if (boostSetting.getInput() == 0 && (!floatMode || verticalMotion.getInput() == 0)) {
        ...
        boolean verticalFlight = mode.getInput() == 0 && temporaryFlightKey();
        int maximumBoostTicks = verticalFlight && !notMoving ? 32 : 33;
        if (stopTime == -1 && ++boostTicks > maximumBoostTicks) {
            disabled();
            return;
        }
    ...
    private boolean temporaryFlightKey() {
        if (mode.getInput() != 0) return false;
